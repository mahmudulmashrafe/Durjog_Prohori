...
const Profile = () => {
  ...
  // Use mock data as fallback if ngo data from context is not available
  const ngo = ngoFromContext || mockNGOData;
  ...
  const [formData, setFormData] = useState({
    ...
    specializedAreas: ngo?.specializedAreas?.join(', ') || '',
    resources: ngo?.resources?.join(', ') || '',
  ...
  useEffect(() => {
    const loadFullProfile = async () => {
      ...
      if (ngoFromContext && 
          (!ngoFromContext.location || 
          ngoFromContext.latitude === undefined || 
          ngoFromContext.longitude === undefined || 
          !ngoFromContext.phoneNumber)) {
  ...
  useEffect(() => {
    ...
    if (isEditing && ngo) {
      setFormData({
        ...
        resources: ngo.resources?.join(', ') || '',
        location: ngo.location || '',
        latitude: ngo.latitude !== undefined ? ngo.latitude : '',
        longitude: ngo.longitude !== undefined ? ngo.longitude : ''
      });
    }
  }, [isEditing, ngo]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if ((locationData.latitude && !locationData.longitude) || (!locationData.latitude && locationData.longitude)) {
      ...
      const updatedProfile = {
        ...formData,
        ...locationData,
        specializedAreas: formData.specializedAreas.split(',').map(area => area.trim()).filter(area => area),
      ...
      if (result.success) {
        ...
        // Refresh the profile data to make sure we have the latest values
        await refreshProfile();
        setIsEditing(false);
      } else {
        ...
        <div className="bg-gradient-to-r from-green-500 to-green-600 px-6 py-2">
          <div className="flex justify-between items-center">
            <h1 className="text-xl font-bold text-white">
              {language === 'en' ? 'NGO Profile' : 'এনজিও প্রোফাইল'}
            </h1>
        ...
        <div className="px-2 pb-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-1">
            <div className="bg-gray-50 p-4 rounded-lg border border-gray-200 h-full">
              <h3 className="text-xs uppercase tracking-wider text-green-600 mb-2 font-semibold">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  {isEditing ? (
                    <input
                      ...
                      className="w-full mt-1 px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all"
                    ...
                    <div className="text-lg font-medium mt-1 flex items-center text-gray-800">
                      ...
                      {ngo?.name || (language === 'en' ? 'Not provided' : 'প্রদান করা হয়নি')}
                    </div>
                  )}
                </div>
                ...
                <div>
                  ...
                  {isEditing ? (
                    ...
                    <div className="text-lg font-medium mt-1 flex items-center text-gray-800">
                      ...
                      {ngo?.email || (language === 'en' ? 'Not provided' : 'প্রদান করা হয়নি')}
                    </div>
                  )}
                </div>
                ...
                <div>
                  ...
                  {isEditing ? (
                    ...
                    <div className="text-lg font-medium mt-1 flex items-center text-gray-800">
                      ...
                      {ngo?.phoneNumber || (language === 'en' ? 'Not provided' : 'প্রদান করা হয়নি')}
                    </div>
                  )}
                </div>
                ...
                <div>
                  ...
                  <div className="text-lg font-medium mt-1 flex items-center text-gray-800">
                    ...
                    {ngo?.username || (language === 'en' ? 'Not provided' : 'প্রদান করা হয়নি')}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="text-lg font-medium mt-1 flex items-center text-gray-800">
                    ...
                    {ngo?.role ? (
                      language === 'en' ? 
                        ngo.role.charAt(0).toUpperCase() + ngo.role.slice(1) : 
                        ngo.role === 'admin' ? 'অ্যাডমিন' : 
                        ngo.role === 'coordinator' ? 'সমন্বয়কারী' : 
                        ngo.role === 'volunteer' ? 'স্বেচ্ছাসেবক' : 'সদস্য'
            ...
            <div className="bg-gray-50 p-4 rounded-lg border border-gray-200 h-full">
              <h3 className="text-xs uppercase tracking-wider text-green-600 mb-2 font-semibold">
                {language === 'en' ? 'Organization Information' : 'সংগঠন তথ্য'}
              </h3>
              
              <div className="space-y-4">
                <div>
                  <div className="text-xs text-gray-500">{language === 'en' ? 'Organization' : 'সংগঠন'}</div>
                  {isEditing ? (
                    <input
                      type="text"
                      name="organization"
                      ...
                      onChange={handleChange}
                      className="w-full mt-1 px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all"
                      placeholder={language === 'en' ? 'Enter organization name' : 'সংগঠনের নাম লিখুন'}
                    />
                  ) : (
                    <div className="text-lg font-medium mt-1 flex items-center text-gray-800">
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2 text-green-500" viewBox="0 0 20 20" fill="currentColor">
                      ...
                      {ngo?.organization || (language === 'en' ? 'Not provided' : 'প্রদান করা হয়নি')}
                    </div>
                  )}
                </div>
                
                <div>
                  <div className="text-xs text-gray-500">{language === 'en' ? 'Registration Number' : 'নিবন্ধন নম্বর'}</div>
                  {isEditing ? (
                    <input
                      type="text"
                    ...
                    <div className="text-lg font-medium mt-1 flex items-center text-gray-800">
                      ...
                      {ngo?.registrationNumber || (language === 'en' ? 'Not provided' : 'প্রদান করা হয়নি')}
                    </div>
                  )}
                </div>
                  ...
                  {isEditing ? (
                    <div className="flex items-center mt-1">
                      <input
                        ...
                        onChange={handleChange}
                        className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all"
                      ...
                      <button
                        ...
                        className="ml-2 p-2 bg-green-500 text-white rounded-md hover:bg-green-600 transition-all"
                        title={language === 'en' ? 'Select on Map' : 'মানচিত্রে অবস্থান নির্বাচন করুন'}
                      >
                    ...
                    <div className="text-lg font-medium mt-1 flex items-center text-gray-800">
                      ...
                      {ngo?.location || (language === 'en' ? 'Not provided' : 'প্রদান করা হয়নি')}
                    </div>
                  )}
                </div>
                  ...
                  <div className="flex-1">
                    ...
                    {isEditing ? (
                      ...
                      <div className="text-lg font-medium mt-1 flex items-center text-gray-800">
                        ...
                        {ngo?.latitude !== undefined ? ngo.latitude : (language === 'en' ? 'Not provided' : 'প্রদান করা হয়নি')}
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex-1">
                    ...
                    {isEditing ? (
                      ...
                      <div className="text-lg font-medium mt-1 flex items-center text-gray-800">
                        ...
                        {ngo?.longitude !== undefined ? ngo.longitude : (language === 'en' ? 'Not provided' : 'প্রদান করা হয়নি')}
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div>
                  <div className="text-xs text-gray-500">{language === 'en' ? 'Account Status' : 'অ্যাকাউন্ট স্ট্যাটাস'}</div>
                  <div className="text-lg font-medium mt-1 flex items-center text-gray-800">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2 text-green-500" viewBox="0 0 20 20" fill="currentColor">
                      <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
                    </svg>
                    <span className={`px-2 py-1 text-xs font-semibold rounded-full ${
                      ...
                      ngo.status === 'inactive' ? 'bg-red-100 text-red-800' :
                      'bg-yellow-100 text-yellow-800'
                    }`}>
                      {ngo?.status ? (
                        language === 'en' ? 
                          ngo.status.charAt(0).toUpperCase() + ngo.status.slice(1) : 
                          ngo.status === 'active' ? 'সক্রিয়' : 
                          ngo.status === 'inactive' ? 'নিষ্ক্রিয়' : 'স্থগিত'
                      ) : (language === 'en' ? 'Unknown' : 'অজানা')}
                    </span>
                  </div>
                </div>
              </div>
